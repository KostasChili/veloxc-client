...
export const usersApiSlice = apiSlice.injectEndpoints({ //we use the api slice to inject the endpoints in the original api slice
    endpoints:builder=>({
        getUsers:builder.query({
            ...
            transformResponse:responseData=>{                          //responseData is the response from the query
                const loadedUsers = responseData.map(user =>{          //we map over the Data and we set the user.id to user._id (mongo id)
                    user.id = user._id
                    return user
                });
        ...
        addNewUser:builder.mutation({                                   //we pass in some initial data
            query:initialUserData=>({                                   //we go to the users endpoint with a POST method  
                url:'/users',
                method:'POST',
                body:{                                                  //and pass this data to the body
                    ...initialUserData,
                }
            }),
            invalidatesTags:[                                           //we invalidate the data forcing the cache we use with RTK
        ...
        updateUser:builder.mutation({
            ...
            invalidatesTags:(result,error,arg)=>[                      //now we can specify the id of te user we update
